...
public class SocketPrototype : MonoBehaviour
{
	...
	{
		get{ if(threadset.IsConnected) { return threadset.socketId; } 
	...
	private void OnLeaveServerRoom(JSONObject jsonObject)
	{
		...
		UserJson userJson = JsonSerializer.ToObject<UserJson>(jsonObject.ToString());

		for(int i = 0; i < socketIdentities.Count; i++)
		{
			Destroy (socketIdentities[i].gameObject);
		}

		socketIdentities = new List<SocketIdentity>();
	}
	...
	// ----------------------------------------- SENDING TRANSFORM ROOM VALUE! --------------------------------------- //

	public List<SocketIdentity> socketIdentity = new List<SocketIdentity>();
	public List<SocketIdentity> socketIdentities = new List<SocketIdentity>();

	public void TranSync(TransJson transJson)
	{
		string sendData = JsonUtility.ToJson (transJson);
	...
	{
		Debug.Log ("OnEvent:" + "INIT HERE: " + " Content: " + jsonObject.ToString());
		Instances instances = JsonSerializer.ToObject<Instances>(jsonObject.ToString());

		SocketIdentity curUser = Instantiate (socketIdentity[instances.prefabIndex], instances.pos.ToVector, instances.rot.ToQuaternion);
		curUser.IsLocalUser = true; curUser.socketId = instances.identity; curUser.uSocketNet = this;
		socketIdentities.Add (curUser);
	}

	private void OnInstanceOther(SocketIOEvent _eventArgs)
	{
		Debug.Log ("OnEvent:" + "INIT OTHER: " + " Content: " + _eventArgs.data.ToString());
	...
	private void OnStateReceived(SocketIOEvent _eventArgs)
	{
		if(_eventArgs.data.keys.Exists(x => x == "identity"))
		{
			...
			{
				SocketIdentity sockId = socketIdentities.Find (x => x.socketId == identity);
	...
	void Awake()
	{
		...
		CallbackOn ("instanced", OnInstanceOther);
		CallbackOn ("transtated", OnStateReceived);

		//CallbackOn ("created", SocketCallback.Access.OnRoomCreated);

		//CallbackOn ("pubchaposs", SocketCallback.Access.OnRoomPosReceived); //CallbackOn ("pubchaposs", SocketCallback.Access.OnRoomPosReceived);
	...
	void Update()
	{
		...
		if(threadset.IsInitialized)
		{
			...
			{ return; } queueCoder.ackList.RemoveAt(0);
		}
	}

	#endregion

	#region WebSocket Threading Interface
	...
	private void HandleMessage(Packet packet)
	{
		...
		if(packet.socketPacketType == SocketPacketType.ACK)
		{
			...
			{
				if(queueCoder.ackList[i].packetId != packet.id){ continue; }
				lock(queueCoder.ackQueueLock){ queueCoder.ackQueue.Enqueue(packet); } return;
			}
		}
	...
	private void EmitEvent(SocketIOEvent eventArgs)
	{
		...
		{ try { handler(eventArgs); } catch(Exception except) { Debug.LogWarning (except.Message); } }
	}
	...
	private void CallbackOff(string events, Action<SocketIOEvent> callback)
	{
		...
		{ Debug.LogWarning ("No callbacks registered for event: " + events); return; }

		List<Action<SocketIOEvent>> eventList = queueCoder.handlers [events]; 
		if (!eventList.Contains(callback))
		{ Debug.LogWarning ("Couldn't remove callback action for event: " + events); return; }
